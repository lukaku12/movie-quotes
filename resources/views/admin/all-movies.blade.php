<x-layout>
    <div class="flex">
        ...
        <div class="px-12 h-full overflow-auto w-full flex flex-col items-center">
            <h1 class="text-4xl my-5 text-amber-50">{{ __('ui.all_movies') }}</h1>
                    ...
                    <div class="w-72">
                        ...
                        <div class="bg-amber-50 p-3 w-full border border-slate-900 rounded-md rounded-t-none">
                            ...
                            <div class="flex justify-between w-full border-t pt-1">
                                <a href="movies/{{ $movie->id }}/edit" class="bg-amber-300 py-1 px-2 rounded-md hover:bg-amber-500">{{ __('ui.edit') }}</a>
                                <form action="/admin/movies/{{ $movie->id }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="bg-amber-300 py-1 px-2 rounded-md hover:bg-amber-500">{{__('ui.delete')}}</button>
                                </form>
                            </div>
                        </div>
                    </div>
